...
const CONTRACTION_EXPANSIONS = new Map([
  ["don't", ['do', 'not']],
  ["doesn't", ['does', 'not']],
  ["didn't", ['did', 'not']],
  ["isn't", ['is', 'not']],
...
function collapseCompoundNumbers(tokens) {
  const out = [];
...
function normalizeSingleToken(t) {
...
function attachRewriteCharSpans(findings, rewrite) {
...
function tokenCharSpans(text, normTokens) {
  const raw = tokenize(text);
  const collapsed = collapseStammers(raw);
  const withoutFillers = stripFillers(collapsed);
  const spans = [];
